...
#ifdef UNITY_GPU_DRIVEN_PIPELINE
#define SAMPLE_TEXTURE2D_LIGHTMAP SAMPLE_TEXTURE2D_GRAD
#else
    ...
    UNITY_GDRP_DDXDDY_PARAMETER)
{
    ...
    {
#ifdef UNITY_GPU_DRIVEN_PIPELINE
        real4 encodedIlluminance = SAMPLE_TEXTURE2D_LIGHTMAP(lightmapTex, lightmapSampler, LIGHTMAP_EXTRA_ARGS_USE, ddx * transform.xy, ddy * transform.xy).rgba;
#else
    ...
    {
#ifdef UNITY_GPU_DRIVEN_PIPELINE
        illuminance = SAMPLE_TEXTURE2D_LIGHTMAP(lightmapTex, lightmapSampler, LIGHTMAP_EXTRA_ARGS_USE, ddx * transform.xy, ddy * transform.xy).rgb;
#else
...
#ifdef UNITY_GPU_DRIVEN_PIPELINE
    real4 direction = SAMPLE_TEXTURE2D_LIGHTMAP(lightmapDirTex, lightmapDirSampler, LIGHTMAP_EXTRA_ARGS_USE, ddx * transform.xy, ddy * transform.xy);
#else
...
// Just a shortcut that call function above
real3 SampleDirectionalLightmap(TEXTURE2D_LIGHTMAP_PARAM(lightmapTex, lightmapSampler),
    TEXTURE2D_LIGHTMAP_PARAM(lightmapDirTex, lightmapDirSampler),
    LIGHTMAP_EXTRA_ARGS,
    float4 transform,
    float3 normalWS,
    bool encodedLightmap,
