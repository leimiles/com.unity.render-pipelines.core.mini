...
#define DECLARE_STACK_CB(stackName) \
    ...
    // For now, the macro, DECLARE_STACK_CB, is just used in ShaderGraph.
    #undef DECLARE_STACK_CB
    #define DECLARE_STACK_CB(stackName, index) \
        UNITY_DEFINE_GDRP_PROP(float4, stackName##_atlasparams, index)

    #define GDRP_ACCESS_STACK(stackName, index) \
        UNITY_GDRP_MATERIAL_LOAD_ARRAY(float4,\
        UNITY_GDRP_MATERIAL_PAGE_OFFSET,\
        UNITY_GDRP_PROP_OFFSET(stackName##_atlasparams),\
        index)
#else
    #define GDRP_ACCESS_STACK(stackName, index) stackName##_atlasparams[index]
#endif
...
\
GraniteTilesetConstantBuffer GetConstantBuffer_##stackName(UNITY_GDRP_MATERIAL_PAGE_OFFSET_UNIQUE_PARAMETER) \
{ \
    int idx = (int) GDRP_ACCESS_STACK(stackName, 1).w; \
    \
...
StackInfo PrepareVT_##stackName(VtInputParameters par UNITY_GDRP_MATERIAL_PAGE_OFFSET_PARAMETER)\
{\
    GraniteStreamingTextureConstantBuffer textureParamBlock;\
    textureParamBlock.data[0] = GDRP_ACCESS_STACK(stackName, 0); \
    textureParamBlock.data[1] = GDRP_ACCESS_STACK(stackName, 1); \
\
    GraniteTilesetConstantBuffer graniteParamBlock = GetConstantBuffer_##stackName(UNITY_GDRP_MATERIAL_PAGE_OFFSET_UNIQUE_ARGUMENT); \
\
    GraniteConstantBuffers grCB;\
...
#define DECLARE_BUILD_PROPERTIES(stackName, layers, layer0Index, layer1Index, layer2Index, layer3Index)\
    VTProperty BuildVTProperties_##stackName(UNITY_GDRP_MATERIAL_PAGE_OFFSET_UNIQUE_PARAMETER)\
    {\
