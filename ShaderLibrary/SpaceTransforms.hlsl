...
float3 TransformWorldToObject(float3 positionWS, float4x4 worldToObjectMatrix)
{
    return mul(worldToObjectMatrix, float4(positionWS, 1.0)).xyz;
}
...
float3 TransformWorldToObjectDir(float3 dirWS, float4x4 worldToObjectMatrix, bool doNormalize = true)
{
    float3 dirOS = mul((float3x3)worldToObjectMatrix, dirWS);
    if (doNormalize)
        return normalize(dirOS);
...
float3 TransformWorldToObjectNormal(float3 normalWS, float4x4 objectToWorldMatrix, float4x4 worldToObjectMatrix, bool doNormalize = true)
{
#ifdef UNITY_ASSUME_UNIFORM_SCALING
    return TransformWorldToObjectDir(normalWS, worldToObjectMatrix, doNormalize);
#else
    // Normal need to be multiply by inverse transpose
    float3 normalOS = mul(normalWS, (float3x3)objectToWorldMatrix);
    if (doNormalize)
        return SafeNormalize(normalOS);
...
real3x3 CreateTangentToWorld(real3 normal, real3 tangent, real flipSign, real oddNegativeScale)
{
    // For odd-negative scale transforms we need to flip the sign
    real sgn = flipSign * oddNegativeScale;
...
{
    return CreateTangentToWorld(normal, tangent, flipSign, GetOddNegativeScale());
}
...
#ifndef UNITY_SPACE_TRANSFORMS_INCLUDED_GDRP
#define UNITY_SPACE_TRANSFORMS_INCLUDED_GDRP

float3 TransformWorldToObjectGDRP(float3 positionWS, float4x4 worldToObjectMatrix)
{
    return mul(worldToObjectMatrix, float4(positionWS, 1.0)).xyz;
}

float3 TransformWorldToObjectDirGDRP(float3 dirWS, float4x4 worldToObjectMatrix, bool doNormalize = true)
{
    float3 dirOS = mul((float3x3)worldToObjectMatrix, dirWS);
    if (doNormalize)
        return normalize(dirOS);
...
float3 TransformWorldToObjectNormalGDRP(float3 normalWS, float4x4 objectToWorldMatrix, float4x4 worldToObjectMatrix, bool doNormalize = true)
{
#ifdef UNITY_ASSUME_UNIFORM_SCALING
    return TransformWorldToObjectDirGDRP(normalWS, worldToObjectMatrix, doNormalize);
#else
    // Normal need to be multiply by inverse transpose
    float3 normalOS = mul(normalWS, (float3x3)objectToWorldMatrix);
    if (doNormalize)
