...
namespace UnityEditor.Rendering
{
    ...
    public static partial class CameraUI
    {
        public static partial class Rendering
        {
            ...
            /// <summary>Draws backface Culling related fields on the inspector</summary>
            /// <param name="p"><see cref="ISerializedCamera"/> The serialized camera</param>
            /// <param name="owner"><see cref="Editor"/> The editor owner calling this drawer</param>
            public static void Drawer_Rendering_BackfaceCulling(ISerializedCamera p, Editor owner)
            {
#if UNITY_GPU_DRIVEN_PIPELINE
                using (new EditorGUI.DisabledScope(!GraphicsSettings.enableGDRP))
                {
                    EditorGUILayout.PropertyField(p.baseCameraSettings.backfaceCulling, Styles.backfaceCulling);
                }
#endif
            }

            public static void Drawer_Rendering_EnableGDRP(ISerializedCamera p, Editor owner)
            {
#if UNITY_GPU_DRIVEN_PIPELINE
/*
 *  Temporarily hide enableGDRP UI on camera. 
 *  TODO: support camera relative rendering
                using (new EditorGUI.DisabledScope(!GraphicsSettings.enableGDRP))
                {
                    EditorGUILayout.PropertyField(p.baseCameraSettings.enableGDRP, Styles.enableGDRPContent);
                }
*/     
#endif
            }
